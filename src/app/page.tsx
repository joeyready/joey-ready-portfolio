...
const heroTiles = [
  { label: 'Product', bg: '#1f2a1a' },
  { label: 'Lifestyle', bg: '#1a1f2a' },
  { label: 'Portrait', bg: '#2a1a1f' },
];

const featured = [
  { cat: 'Product' as Category, title: 'Blush Wellness', bg: '#1f2a1a', span: true },
  { cat: 'Lifestyle' as Category, title: 'Charlotte Stone', bg: '#1a1f2a', span: false },
  { cat: 'Portrait' as Category, title: 'Editorial Series', bg: '#2a1a1f', span: false },
  { cat: 'Food & Bev' as Category, title: 'Better Booch', bg: '#2a241a', span: false },
  { cat: 'Event' as Category, title: 'Findings Market', bg: '#1a202a', span: false },
];
...
export default function Home() {
  return (
    <>
      {/* HERO */}
      <section style={{ minHeight: '100vh', display: 'grid', gridTemplateColumns: '1fr 1fr' }} className="hero-section">
        <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'flex-end', padding: '0 64px 80px', zIndex: 2 }} className="hero-left">
          <motion.p custom={0} variants={fadeUp} initial="hidden" animate="show"
            style={{ fontSize: 10, letterSpacing: '0.3em', textTransform: 'uppercase', color: 'var(--accent)', marginBottom: 24 }}>
            Los Angeles · Commercial Photography
          </motion.p>
          <motion.h1 custom={1} variants={fadeUp} initial="hidden" animate="show"
            style={{ fontFamily: 'var(--font-display)', fontSize: 'clamp(60px, 7vw, 110px)', fontWeight: 300, lineHeight: 0.95, marginBottom: 32 }}>
            Joey<br />
            <em style={{ fontStyle: 'italic', color: 'var(--accent)', display: 'block' }}>Ready.</em>
          </motion.h1>
          <motion.p custom={2} variants={fadeUp} initial="hidden" animate="show"
            style={{ fontSize: 12, lineHeight: 1.8, color: 'var(--muted)', maxWidth: 320, marginBottom: 48 }}>
            Photographer, videographer &amp; content creator. Crafting visual stories for brands, products, and the moments between.
          </motion.p>
        ...
        <div style={{ position: 'relative', overflow: 'hidden' }} className="hero-right">
          <div style={{ position: 'absolute', inset: 0, display: 'grid', gridTemplateRows: '60% 40%', gap: 3 }}>
            <div style={{ background: heroTiles[0].bg, position: 'relative' }}>
              <span className="tile-label">{heroTiles[0].label}</span>
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 3 }}>
              {heroTiles.slice(1).map(t => (
                <div key={t.label} style={{ background: t.bg, position: 'relative' }}>
      ...
      {/* FEATURED WORK */}
      <section style={{ padding: '120px 64px' }} className="featured-section">
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr', gridTemplateRows: '360px 240px', gap: 4 }} className="featured-grid">
          {featured.map((item) => (
            <Link key={item.title} href={`/work/${catSlug(item.cat)}`}
              style={{ position: 'relative', overflow: 'hidden', background: item.bg, gridColumn: item.span ? '1' : undefined, gridRow: item.span ? '1 / 3' : undefined, display: 'block', textDecoration: 'none' }}
      ...
      <style>{`
        ...
        .tile-label { position:absolute; bottom:16px; left:16px; font-size:9px; letter-spacing:0.2em; text-transform:uppercase; color:rgba(232,228,222,0.4); }
        ...
        @media(max-width:1024px){
          ...
          .hero-left{padding:140px 40px 80px!important}
        ...
        @media(max-width:640px){
          .hero-left{padding:120px 24px 60px!important}
          .featured-section,.categories-section,.about-section{padding:80px 24px!important}
